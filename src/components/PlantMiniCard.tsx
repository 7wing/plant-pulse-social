...
export default function PlantMiniCard({ image, name, species, waterDays, healthPercent }: PlantMiniCardProps) {
  ...
  return (
    <div
      className="bg-card rounded-2xl shadow-card overflow-hidden hover:shadow-elevated transition-shadow duration-300 cursor-pointer"
      onClick={() => navigate(`/plant/${name.toLowerCase().replace(/\s+/g, "-")}`)}
    >
